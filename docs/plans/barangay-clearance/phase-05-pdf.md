...
```typescript
const response = await api.get(`/me/clearances/${id}/pdf`, {
  responseType: "blob",
});
...
- [x] `GET /clearances/{id}/pdf` for RELEASED clearance → 200 with `Content-Type: application/pdf`
- [x] PDF contains: clearance number, issuance date, resident full name, age, birthdate, address, purpose
- [x] PDF contains: barangay name, municipality, province, captain name
- [x] If logo uploaded in settings → logo appears in PDF header
- [x] If no logo → header shows text only (no exception)
- [x] `GET /me/clearances/{id}/pdf` for non-RELEASED clearance → 403
- [x] `ClearancePdfServiceTest` generates non-null byte array starting with `%PDF` magic bytes
